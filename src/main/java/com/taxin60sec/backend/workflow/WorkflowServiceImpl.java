...
public void logEvent(
    ...
    private void synchronizeStates(
        ...
        switch (stage) {
            ...
            case COMPLETED -> {
                ...
                completeLinkedObligations(taxCase);
            }
    ...
    private void completeLinkedObligations(Case taxCase) {

        List<ComplianceObligation> linked =
                complianceObligationRepository.findByRelatedCaseIdAndDeletedFalse(taxCase.getId());

        for (ComplianceObligation obligation : linked) {

            if (obligation.getStatus() == ComplianceStatus.COMPLETED) {
                continue;
            }

            obligation.setStatus(ComplianceStatus.COMPLETED);
            obligation.setCompletedAt(Instant.now());
        }

        complianceObligationRepository.saveAll(linked);
    }
