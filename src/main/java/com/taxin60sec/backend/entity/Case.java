...
public class Case extends BaseEntity {
    ...
    /**
     * Set the moment the client confirms this case's work is delivered to their
     * satisfaction — the client-confirmation milestone EscrowService gates a normal
     * (non-admin-override) payout release on. Distinct from completedAt (which marks the
     * CA/workflow side finishing the case) since the client may take time to review before
     * confirming, and an admin can still release escrow without this being set at all.
     */
    private Instant confirmedByClientAt;

    private Instant assignedAt;

    /** Set the moment a CA is first assigned to this case — basis for the "60-second response" guarantee. */
