...
public class ServiceOfferingMapper {
    public ServiceOfferingDto toDto(ServiceOffering serviceOffering) {
        return new ServiceOfferingDto(
                ...
                serviceOffering.getComplexity(),
                serviceOffering.getIncludedFeatures() == null || serviceOffering.getIncludedFeatures().isBlank() ? java.util.List.of() : java.util.Arrays.stream(serviceOffering.getIncludedFeatures().split("\\r?\\n")).filter(q -> !q.isBlank()).toList(),
