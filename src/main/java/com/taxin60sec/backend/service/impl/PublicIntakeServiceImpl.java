...
public class PublicIntakeServiceImpl implements PublicIntakeService {
    ...
    private final com.taxin60sec.backend.repository.HealthCheckLeadRepository healthCheckLeadRepository;
    private final ComplianceObligationGeneratorService complianceObligationGeneratorService;
    ...
    public PublicIntakeServiceImpl(
            ...
            com.taxin60sec.backend.repository.HealthCheckLeadRepository healthCheckLeadRepository,
            ComplianceObligationGeneratorService complianceObligationGeneratorService,
        ...
        this.healthCheckLeadRepository = healthCheckLeadRepository;
        this.complianceObligationGeneratorService = complianceObligationGeneratorService;
    ...
    public PublicStartResponse start(PublicStartRequest request) {
        ...
        if (request.healthCheckLeadId() != null) {
            healthCheckLeadRepository.findById(request.healthCheckLeadId()).ifPresent(lead -> {
                lead.setClientId(client.getId());
                lead.setCaseId(taxCase.getId());
                lead.setConverted(true);
                healthCheckLeadRepository.save(lead);

                complianceObligationGeneratorService.generateFromHealthCheckLead(
                        client, taxCase, lead.getTriggeredServiceCodes()
                );
            });
        }
        ...
        if (client.getEmail() != null && !client.getEmail().isBlank()) {
            try {
                notificationService.sendInstantAcknowledgmentEmail(
                        client.getEmail(),
                        client.getFullName(),
                        service.getDisplayName()
                );
            } catch (Exception ex) {
                log.warn("Failed to send instant acknowledgment email for case {}", taxCase.getId(), ex);
            }
        }

        if (client.getPhoneNumber() != null && !client.getPhoneNumber().isBlank()) {
            try {
                notificationService.sendInstantAcknowledgmentWhatsApp(
                        client.getPhoneNumber(),
                        client.getFullName(),
                        service.getDisplayName()
                );
            } catch (Exception ex) {
                log.warn("Failed to send instant acknowledgment WhatsApp for case {}", taxCase.getId(), ex);
            }
        }
    ...
    /**
     * A user starting the public intake may already exist as a CA/admin (e.g. testing
     * the WhatsApp flow with their own phone number). Promote them to also carry
     * ROLE_CLIENT so their cases show up on the admin Clients screen, and make sure
     * they have a ClientProfile row like any other client.
     */
    private void ensureClientRole(User user) {
        boolean alreadyClient = user.getRoles().stream()
        ...
        clientProfileRepository.findByUserId(user.getId()).orElseGet(() -> {
    ...
    private User createClient(PublicStartRequest request) {
        ...
        if (request.referralCode() != null && !request.referralCode().isBlank()) {
            ...
            String code = request.referralCode().trim().toUpperCase();
            user.setReferredByCode(code);

            userRepository.findByReferralCode(code).ifPresent(referrer -> {
                referrer.setReferralCredits(referrer.getReferralCredits() + 1);
    ...
    @Override
    @Transactional
    public com.taxin60sec.backend.dto.publicintake.SubmitCaseResponse submitCase(Long caseId) {
        ...
        User client = referralService.ensureReferralCode(taxCase.getClient());

        return new com.taxin60sec.backend.dto.publicintake.SubmitCaseResponse(
                client.getReferralCode(),
                publicUrl + "/?ref=" + client.getReferralCode()
        );
